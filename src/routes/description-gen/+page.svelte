...
<script lang="ts">
  ...
  let lastId = 4;
  let inputValues: Record<number, { feature: string; value: string }> = {};
  ...
  function addItem() {
    ...
    items = [...items, newItem];
    inputValues[newItem.id] = { feature: "", value: "" }; // Add the new item to inputValues
  }
  ...
  const handleConsider = (evt: CustomEvent) => {
    items = evt.detail.items;
  };

  const handleFinalize = (evt: CustomEvent) => {
    items = evt.detail.items;
  };
              ...
              <div class="card card-hover my-4 flex gap-2 items-center justify-evenly p-2 text-md " animate:flip="{{ duration:flipDurationMs }}" >
                ...
                <div class="input-forms flex flex-col md:flex-row gap-2 ml-2">
                  ...
                  <div class="input-group h-10 border border-primary-50 input-group-divider grid-cols-[auto_1fr_auto]">
                    ...
                    <input type="text" placeholder="" bind:value={item.value} />
                  </div>
                </div>
      ...
      <div class="flex-1 flex flex-col gap-5 w-full lg:w-2/5 mt-10 md:mt-0">
        ...
        <div class="space-y-2 my-2 text-md text-primary-500">
          <label class="flex items-center space-x-2">
            <input class="checkbox w-4 h-4" type="checkbox" bind:checked={padTipChecked} />
            <p>Add tip/suggestion for Supa Rug Pad Grip</p>
          </label>
          <label class="flex items-center space-x-2">
            <input class="checkbox w-4 h-4" type="checkbox" bind:checked={colorWarningChecked} />
